...
#define print_work_info(where, how, wrap) \
{ \
    ...
    printk("[myirq][" where "(%d)][" how "] work info | time : %lld:%lld:%lld", \
           wrap->id, h, m, s); \
}

void work_handler(struct work_struct *w)
{
    struct work_wrap *wrap = container_of(w, struct work_wrap, work);

    if (wrap->delay)
    {
        print_work_info("work_handler", "before delay", wrap);
        msleep(wrap->delay);
        print_work_info("work_handler", "after delay", wrap);
    }

    print_work_info("work_handler", "", wrap);
    printk("[myirq][work_handler(%d)] keyboard scan code: %lu (%s)", wrap->id,
           wrap->code, get_scan_code(wrap->code));
}
...
irqreturn_t irq_handler(int line, void *data)
{
    ...
    if (IRQ_NUM == line)
    {
        ...
        print_work_info("irq_handler", "before schedule work", (&work1));
        queue_work(workqueue, &work1.work);
        print_work_info("irq_handler", "after schedule work", (&work1));
        print_work_info("irq_handler", "before schedule work", (&work2));
        queue_work(workqueue, &work2.work);
        print_work_info("irq_handler", "after schedule work", (&work2));
...
{
    int rc = request_irq(IRQ_NUM, irq_handler, IRQF_SHARED, "mykbd-handler-workqueue", &dev_id);
    ...
    if (!(workqueue = alloc_workqueue("kdb-workqueue", WQ_FREEZABLE, 1)))
    {
        printk("[myirq] Unable to allocate workqueue");
        return 1;
    }

    INIT_WORK(&work1.work, work_handler);
    INIT_WORK(&work2.work, work_handler);
